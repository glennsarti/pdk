...
module PDKCorePlugins
  class ReleaseCLIPlugin < PDK::PluginTypes::CLI
    def initialize
      super('release_cli')
    end
    ...
    def create_cri_command
      base_cri_command.define_command do
        ...
        run do |opts, _args, _cmd|
          ...
          PDKCorePlugins::ReleaseCLIPlugin.send_analytics('release', opts)
    ...
    class << self
      ...
      def module_compatibility_checks!(release, opts)
      ...
      def send_analytics(command, opts)
      ...
      def prepare_version_interview(prompt, opts)
        questions = [
          {
            name:             'version',
      ...
      def prepare_publish_interview(prompt, opts)
...
# Add in our release process
# TODO: This probably isn't a good idea
module PDK
  module Module
    autoload :Release, 'pdk/plugins/release/module/release'
  end
end

PDK::PluginManager.instance.register(PDKCorePlugins::ReleaseCLIPlugin.new)
