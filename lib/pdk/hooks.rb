...
module PDK
  # This code was inspired from Puppet-Debugger (https://raw.githubusercontent.com/nwops/puppet-debugger/master/lib/puppet-debugger/hooks.rb) which was itself inspired from Pry hooks file
  ...
  class HookManager
    ...
    def exec_hook(event_name, *args, &block)
      PDK::Logger.debug("Starting to execute hook #{event_name}") # unless event_name == :hook_log_message
      ...
      PDK::Logger.debug("Finished executing hook #{event_name}") # unless event_name == :hook_log_message
    end
